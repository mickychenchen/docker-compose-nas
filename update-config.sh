...
function update_bazarr_config {
    ...
    sed -i.bak "s/use_radarr4k: false/use_radarr4k: true/" "${CONFIG_ROOT:-.}"/"$container"/config/config/config.yaml && rm "${CONFIG_ROOT:-.}"/"$container"/config/config/config.yaml.bak
    ...
    until [ -f "${CONFIG_ROOT:-.}"/radarr4k/config.xml ]; do sleep 1; done
    RADARR4K_API_KEY=$(sed -n 's/.*<ApiKey>\(.*\)<\/ApiKey>.*/\1/p' "${CONFIG_ROOT:-.}"/radarr4k/config.xml)
    sed -i.bak "/radarr4k:/ { s/apikey: .*/apikey: $RADARR4K_API_KEY/; s/base_url: .*/base_url: \/radarr4k/; s/ip: .*/ip: radarr4k/ }" "${CONFIG_ROOT:-.}"/"$container"/config/config/config.yaml && rm "${CONFIG_ROOT:-.}"/"$container"/config/config/config.yaml.bak
...
for container in $(docker ps --format '{{.Names}}'); do
  if [[ "$container" =~ ^(radarr|radarr4k|sonarr|lidarr|prowlarr)$ ]]; then
